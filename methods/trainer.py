...
from utils.loss import AdaptiveFeatureDistillation
...
class Trainer:
    def __init__(self, task, device, logger, opts):
        ...
        self.fd_loss = AdaptiveFeatureDistillation(reduction="mean", alpha=1).to(device)
    ...
    def train(self, cur_epoch, train_loader, metrics=None, print_int=10, n_iter=1):
        ...
        for iteration in range(n_iter):
            ...
            for images, labels in train_loader:
                ...
                fad_loss = torch.tensor([0.]).to(self.device)
                optim.zero_grad()
                ...
                if self.model_old is not None:
                    ...
                    fad_loss = self.fd_loss(feat, feat_old, labels=labels, num_classes=self.opts.num_classes, weights=1,
                                        pod=True)
                    print("fad_loss:", fad_loss)
                ...
                if rloss <= CLIP:
                    loss_tot = loss + rloss + fad_loss * 2
                else:
