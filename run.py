...
def main(opts):
    ...
    if task.nshot != -1:
        ...
        logdir_full = f"{opts.logdir}/step{opts.step}/{name}/"
    else:
        logdir_full = f"{opts.logdir}/step{opts.step}/{opts.name}/"
    if rank == 0:
        logger = Logger(logdir_full, rank=rank, debug=opts.debug, filename=name, summary=opts.visualize, step=opts.step)
    else:
        logger = Logger(logdir_full, rank=rank, debug=opts.debug, filename=name, summary=False)

    opts.num_classes = [len(get_tasks(opts.dataset, opts.task, step)) for step in range(opts.step + 1)]
    opts.num_classes = [opts.num_classes[0] - 1] + opts.num_classes[1:]

    if rank == 0:
        logger.info("==============================================")
        logger.info(f"  task : {opts.task}")
        logger.info(f"  step : {opts.step}")
        logger.info("  Device: %s" % device)
        if opts.step > 0:
            logger.info(f'\nCurrent configuration: {opts.nshot}, Random seed: {opts.random_seed}')
        logger.info("  opts : ")
        logger.info(opts)
        logger.info("==============================================")
    ...
    if opts.iter is not None:
        logger.info(f"len(train_loader):{len(train_loader)},train_iterations:{train_iterations}")
